...
# 第3弾プロトタイプ
左にソースコード入力欄（上にHTML、下にCSS、その下にCodeボタン）、右にISOのA判と同じ比率のプレビューエリアを実装

## 処理実装について
* プレビューエリアのレイアウト
    - アスペクト比を`1/1.41414`で指定、はみだしなどについては`overflow: hidden;`で制御
    - **TODO** はみだしに警告文を出したい
* コード入力エリアのレイアウト
    - `textarea`を縦に配置
...
* サニタイズの強化
    - CSSからのXSSへの対策追加するか？
* UXの向上
    - シンタックスハイライトなど、コード入力のUX強化

第4弾はUXの向上（コード入力の利便性強化、リアルタイムプレビューの検討、はみだし警告の実装）を目指す。